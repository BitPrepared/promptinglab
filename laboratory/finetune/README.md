...
golden.jsonl ──▶ validate_dataset.py ──(ok)──▶ train_colab.py [Colab/GPU] ──▶ GGUF
   ▲                                              │
   │  curation                                    └─ drop-in: laboratory/models/*.gguf
...
- compose (mini PC): `MODEL_FILE=diario-1.5b-q4_k_m.gguf`
- CLI standalone (Pi): `MODEL_PATH=models/diario-0.5b-q4_k_m.gguf LAB_BACKEND=local`
