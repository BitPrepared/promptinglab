...
Config via env:
  LAB_BACKEND     = mock | llama | local   (default: mock)
...
def build_skill_from_env() -> DiarioSkill:
    ...
    # rilevare llama automaticamente; si può forzare con LAB_BACKEND.
    backend = os.environ.get("LAB_BACKEND", "mock").lower()
    ...
    if backend in ("llama", "llama-server", "server"):
        ...
        if not model_path:
            raise RuntimeError("LAB_BACKEND=local richiede MODEL_PATH")
