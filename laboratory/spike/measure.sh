...
LLAMA_DIR="$LAB_ROOT/.llamacpp/llama-b10405"
MODEL="${1:?model path required}"
THREADS="${2:?threads required}"
LABEL="${3:?label required}"
PORT_LLAMA=8081
PORT_SKILL=8080
NOTES_FILE="$LAB_ROOT/spike/notes.txt"
LLAMA_LOG="$(mktemp)"
SKILL_LOG="$(mktemp)"
PAYLOAD="$(mktemp)"
OUT="$(mktemp)"
...
LAB_BACKEND=llama LLAMA_URL="http://localhost:$PORT_LLAMA" LAB_PORT="$PORT_SKILL" \
  python3 -m backend.service >"$SKILL_LOG" 2>&1 &
SKILL_PID=$!
...
cp "$OUT" "$LAB_ROOT/spike/out_${LABEL}.json"
