...
```bash
cd diariobot
python3 -m diariobot.service              # servizio HTTP su :8080, backend mock
# in un altro terminale:
python3 -m diariobot.cli "Oggi con Marco e Lucia al campo. Abbiamo montato la tenda."
```
...
> **Backend `auto`** (default nel `docker-compose`): `DIARIOBOT_BACKEND=auto` usa
...
# 3) Puntaci la skill:
export DIARIOBOT_BACKEND=llama
...
> Per il diario di bordo usa la CLI/`service` di diariobot (`:8080`), che
...
nginx (statici + proxy `/api/*`) → gateway (`diariobot.gateway`) → skill/llama.
