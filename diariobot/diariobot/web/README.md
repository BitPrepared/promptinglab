...
DIARIOBOT_BACKEND=auto python3 -m diariobot.service
# terminale 2: gateway (solo /api/*)
SKILL_URL=http://localhost:8080 LLAMA_URL=http://localhost:8081 python3 -m diariobot.gateway
# terminale 3 (opzionale, preview della pagina senza API):
cd diariobot/web/static && python3 -m http.server 8090
```
...
Il compose usa già `DIARIOBOT_BACKEND=auto` di default: basta il profilo model,
...
```
(`DIARIOBOT_BACKEND` è `mock|llama|auto`; con `auto` la skill prova llama e torna
