...
#
# Workflow:
#   1. Su Colab: Runtime -> GPU (T4). Carica diariobot/finetune/dataset/golden.jsonl
#      e questo script.
#   2. Esegui le celle in ordine. Alla fine scarica il GGUF Q4_K_M.
#   3. Copia il GGUF in diariobot/models/ e punta MODEL_FILE li' (compose) o
...
OUT_DIR     = Path(f"./out_diariobot_{TARGET}")
GOLDEN_JSONL = Path("./golden.jsonl")   # carica diariobot/finetune/dataset/golden.jsonl su Colab
MAX_SEQ_LEN  = 2048
EPOCHS       = 3
LR           = 2e-4
LORA_R       = 16
...
# Prompt IDENTICI a diariobot/prompts.py (mantenere sincronizzati).
SYSTEM_PROMPT = (
...
# Esporta GGUF Q4_K_M (pronto per llama.cpp / diariobot). Metodi alternativi in
...
print("\nScarica il file .gguf, copialo in diariobot/models/ (es. diariobot-diario-<TARGET>-q4_k_m.gguf),")
